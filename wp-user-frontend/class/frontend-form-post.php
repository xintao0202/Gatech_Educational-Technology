...
class WPUF_Frontend_Form_Post extends WPUF_Render_Form {
	...
	// check if current user signed-up for this project, if yes, he or she can edit.
	...
	$p_t=get_the_title($post_id); 
	
	if(!empty($t)){
		global $wpdb;
		$current_project= implode(',', $t);
		if ($current_project=='assessment'){
		$current_project=$p_t;
		}
		
		$projects = $wpdb->get_results( "SELECT ID FROM wp_posts WHERE post_title ='$current_project' AND post_type='job_listing'");
		$projectid= $projects[0]->ID;
		//echo $projectid;
		$results = $wpdb->get_results( "SELECT userid FROM " .$wpdb->prefix . "SignUpProject WHERE projectid = '$projectid'");
		//print_r($results);
		$userids=[];
	...
	if(isset($userids)){
	if ( !current_user_can( 'delete_others_posts' ) or ( !in_array($username, $userids) )) {
            return '<div class="wpuf-info">' . __( 'You are not allowed to edit', 'wpuf' ) . '</div>';
        }
	}
